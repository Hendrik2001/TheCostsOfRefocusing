...
*file with companyid, div_cons1, div_cons2, div_corr, corr_eps, and div_corr_id

keep div_cons1
rename div_cons1 id
save divcorr_id1.dta, replace
use complete_divcorr
keep div_cons2 
rename div_cons2 id
append using divcorr_id1.dta
bysort id: gen counter=_n
keep if counter==1
drop counter
sort id

save potential_treat1.dta, replace
unique id
...
merge m:1 id using potential_treat1.dta
...
save potential_treat2.dta, replace
...
replace intheset2 = 0 if mydate<=625 & mydate >=617
...
keep id mydate intheset intheset2
...
save integrate2.dta, replace

clear

use potential_treat2

keep if closedxcrisis==1

keep id

save closed.data, replace
...
merge 1:m id using integrate2.dta
keep if _merge==3
drop _merge


g treatment99 = 0
replace treatment99 = 1 if div_corr<0.985

g treatment90 = 0
replace treatment90 = 1 if div_corr<0.895

g treatment99_eps = 0
replace treatment99_eps = 1 if corr_eps<0.985 & corr_eps !=.

g treatment90_eps = 0
replace treatment90_eps = 1 if corr_eps<0.895 & corr_eps !=.
...
erase batch3.dta
erase batch4.dta
erase potential_treat1.dta
erase integrate2.dta
