...
*log using pscore2firm.log, replace
...
collapse (max) treated (mean) firm_ir_minusi Llog_firmscope Llog_firmage Lfirmsz_q*, by(companyid)
...
hotelling firm_ir_minusi Llog_firmscope Llog_firmage Lfirmsz_q*, by(treated)


probit treated firm_ir_minusi Llog_firmscope Llog_firmage Lfirmsz_q*
predict pscore
mfx compute

save just_after_probit_pscore2firm.dta, replace
...
replace upper_bound90 = r(p90)

g lower_bound=0
sum pscore if treated==0, d
replace lower_bound =r(p1)

replace xtreme = 1 if pscore>upper_bound90
...
unique companyid if treated==1

****************************
**** Common Support ****
**************************

keep companyid pscore treated
...
gsort -treated -pscore companyid
...
forvalues t = 1/`end' { 
	*display `t'
	...
	quietly:  egen maxmatched = max(matched), by(companyid)
	quietly:  replace matched_id = 1 if maxmatched==1
	quietly:  replace id_to_match = companyid if counter == `t'
...
keep companyid pscore treated nearest_neighbor
...
save match_pscore2firm.dta, replace
...
append using match2_pscore2firm.dta
append using nearest_neighborfirm.dta

save match_regfirm.dta, replace
...
merge 1:1 companyid using just_after_probit_pscore2firm.dta
drop if _merge !=3
